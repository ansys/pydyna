...
class DynaPre():
    """Contains methods to create keyword related to Dyna Preprocessing"""
    
    def __init__(self):
        init_log('Client/client.log')
        hostname = 'localhost'
        if len(sys.argv) > 1:
            hostname = sys.argv[1]
    ...
    def Openfiles(self,filenames):
        ...
        for filename in filenames:
            ...
            upload(self.stub, filename)
            logging.info(filename + ' uploaded to server...')
        self.mainname = os.path.basename(filenames[0])
        return self.stub.LoadFile(LoadFileRequest())

    def  CreateTimestep(self,tssfac, isdo,dt2ms):
        ...
        bool
            "True" when successful, "False" when failed
        """  
        ret = self.stub.CreateTimestep(TimestepRequest(tssfac = tssfac, isdo = isdo,dt2ms = dt2ms))
        logging.info('Timestep Created...')
        return ret

    def CreateTermination(self,endtim):
    ...
    def CreateContact(self,rwpnal,ignore,igactc):
        ...
        bool
            "True" when successful, "False" when failed
        """ 
        ret = self.stub.CreateContact(ContactRequest(rwpnal = rwpnal, ignore = ignore, igactc = igactc))
        logging.info('Contact Created...')
        return ret

    def CreateDBBinary(self,dt):
    ...
    def CreateRigidWallGeom(self,geomtype, motion, display,parameter, lcid, vx, vy, vz):
        """Create *DATABASE_BINARY_D3PLOT keyword
        Parameters
        ----------
        geomtype : int 
        ...
        bool
            "True" when successful, "False" when failed
        """ 
        ret = self.stub.CreateRigidWallGeom(RigidWallGeomRequest(geomtype = geomtype, motion = motion, display = display,parameter = parameter, lcid = lcid, vx = vx, vy = vy, vz = vz))
        logging.info('Cylinder Rigidwall Geometric Created...') 
        return ret
    
    def CreateDefineCurve(self,lcid, sfo, abscissa, ordinate):
        ...
        bool
            "True" when successful, "False" when failed
        """ 
        ret = self.stub.CreateDefineCurve(DefineCurveRequest(lcid = lcid, sfo = sfo, abscissa = abscissa, ordinate = ordinate))
        logging.info('DefineCurve 1 Created...')
        return ret

    def CreateSectionIGAShell(self,secid,elform,shrf,thickness):
        ...
        bool
            "True" when successful, "False" when failed
        """
        ret = self.stub.CreateSectionIGAShell(SectionIGAShellRequest(secid = secid, elform = elform, shrf = shrf, thickness = thickness))
        logging.info('Section IGAShell 1 Created...')
        return ret
    
    def SetPartProperty(self,pid,secid,mid,eosid,hgid,grav,adpopt,tmid):
        ...
        bool
            "True" when successful, "False" when failed
        """
        ret = self.stub.SetPartProperty(PartPropertyRequest(pid = pid, secid = secid, mid = mid, eosid = eosid, hgid = hgid, grav = grav, adpopt = adpopt, tmid = tmid))
        return ret
    
    def CreatePartSet(self,sid,pids):
        """Create *PART keyword
        Parameters
        ----------
        sid : int
    ...
    def CreateSectionSolid(self,secid,elform):
        ...
        bool
            "True" when successful, "False" when failed
        """
        ret = self.stub.CreateSectionSolid(SectionSolidRequest(secid = secid, elform = elform))
        logging.info('Section Solid Created...')
        return ret
    
    def CreateHourglass(self,ghid, ihq, qm, q1, q2, qb , qw):
    ...
    def CreateContactAutomatic(self,ssid, msid, sstyp, mstyp, option):
        ...
        bool
            "True" when successful, "False" when failed
        """
        ret = self.stub.CreateContactAutomatic(ContactAutomaticRequest(ssid = ssid, msid = msid, sstyp = sstyp, mstyp = mstyp, option = option))
        logging.info('Contact Automatic  Created...')
        return ret

    def CreateContactTied(self,ssid, msid, sstyp, mstyp):
        ...
        bool
            "True" when successful, "False" when failed
        """
        ret = self.stub.CreateContactTied(ContactTiedRequest(ssid = ssid, msid = msid, sstyp = sstyp, mstyp = mstyp))
