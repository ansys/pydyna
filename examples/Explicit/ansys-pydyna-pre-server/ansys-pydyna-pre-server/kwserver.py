...
if platform.system() == "Windows":
    if sys.version_info[0] == 3 and sys.version_info[1] == 8:
        ...
        sys.path.insert(0, os.path.normpath(os.path.join(cwd, "lib", "windows", "cp311")))
else:
...
class IGAServer(kwprocess_pb2_grpc.kwC2SServicer):
    ...
    def CreateTimestep(self, request, context):
        ...
        lctm = request.lctm
        firstcard = (
            "0.0," + str(tssfac) + "," + str(isdo) + ",0.0," + str(dt2ms) + "," + str(lctm) + ",0,0"
        )
    ...
    def CreateControlEnergy(self, request, context):
        ...
        irgen = request.irgen
        card1 = (
            str(hgen) + "," + str(rwen) + "," + str(slnten) + "," + str(rylen) + "," + str(irgen)
        )
        newk = "*CONTROL_ENERGY\n" + card1
        self.kwdproc.newkeyword(newk)
    ...
    def CreateControlThermalNonlinear(self, request, context):
        tol = request.tol
        dcp = request.dcp
        card1 = "10," + str(tol) + "," + str(dcp)
        newk = "*CONTROL_THERMAL_NONLINEAR\n" + card1
        self.kwdproc.newkeyword(newk)
        print("Control thermal nonlinear Created...")
        return kwprocess_pb2.ControlThermalNonlinearReply(answer=0)
    ...
    def CreateControlImplicitConsistentMass(self, request, context):
        iflag = request.iflag
        card1 = str(iflag)
        newk = "*CONTROL_IMPLICIT_CONSISTENT_MASS\n" + card1
        self.kwdproc.newkeyword(newk)
        print("Control Implicit Consistent Mass Created...")
        return kwprocess_pb2.ControlImplicitConsistentMassReply(answer=0)
    ...
    def CreateInitVelGeneration(self, request, context):
        ...
        card1 = (
            str(id) + "," + str(styp) + "," + str(omega) + "," + str(vx) + str(vy) + "," + str(vz)
        )
        card2 = (
            str(xc)
            + ","
    ...
    def CreateDefineCurve(self, request, context):
        title = request.title
        lcid = request.lcid
        lcid = self.kwdproc.get_data(gdt.KWD_DEFINE_CURVE_LASTID) + self.kwdproc.get_data(gdt.KWD_DEFINE_FUNCTION_LASTID) + 1
        sfo = request.sfo
    ...
    def CreateDefineFunction(self, request, context):
        funcid = self.kwdproc.get_data(gdt.KWD_DEFINE_CURVE_LASTID) + self.kwdproc.get_data(gdt.KWD_DEFINE_FUNCTION_LASTID) + 1
        function = request.function
        card1 = str(funcid)
        card2 = str(function)
    ...
    def CreateDefineFunctionTabulated(self, request, context):
        ...
        abscissa = request.abscissa
        ordinate = request.ordinate
        fields1 = (str(funcid),str(heading))
        card1 = ",".join(fields1)
        card2 = str(function)
        newk = "*DEFINE_FUNCTION_TABULATED\n" + card1 + "\n" +card2 + "\n"
        for index in range(len(abscissa)):
            repeatcard = str(abscissa[index]) + "," + str(ordinate[index]) + "\n"
            newk += repeatcard
        self.kwdproc.newkeyword(newk)
        print(f"DEFINE_FUNCTION_TABULATED {funcid} Created...")
        return kwprocess_pb2.DefineFunctionTabulatedReply(id=funcid)
    
    def CreateDefineTable2D(self, request, context):
        title = request.title
        tid = self.kwdproc.get_data(gdt.KWD_DEFINE_CURVE_LASTID) + 1
        values = request.values
        cids = request.cids
        card1 = str(tid)
        opcode = "*DEFINE_TABLE_2D"
        newk = opcode
        if len(title) > 0:
            newk += "_TITLE" + "\n"
            newk += title
        ...
        for index in range(len(values)):
            repeatcard = str(values[index]) + "," + str(cids[index]) + "\n"
            newk += repeatcard
        self.kwdproc.newkeyword(newk)
        msg = "Define Table 2D " + str(tid) + "Created..."
        print(msg)
        return kwprocess_pb2.DefineTable2DReply(id=tid)
    ...
    def CreateRigidWallGeom(self, request, context):
        ...
        prm = request.parameter
        lcid = request.lcid
        vx = request.vx
        vy = request.vy
        vz = request.vz
        fields = (str(prm[0]),str(prm[1]),str(prm[2]),str(prm[3]),str(prm[4]),str(prm[5]))
        card2 = ",".join(fields)
        if geomtype == 3:
            opcode = "*RIGIDWALL_GEOMETRIC_CYLINDER"
            fields3 = (str(prm[6]),str(prm[7]))
            card3 = ",".join(fields3)
            print("Cylinder Rigidwall Geometric Created...")
        elif geomtype == 4:
            opcode = "*RIGIDWALL_GEOMETRIC_SPHERE"
            card3 = str(prm[6])
            print("Sphere Rigidwall Geometric Created...")
        if motion >= 0:
            card4 = str(lcid) + "," + str(motion) + "," + str(vx) + "," + str(vy) + "," + str(vz)
            opcode += "_MOTION"
        if display != 0:
            opcode += "_DISPLAY"
        newk = opcode + "\n0,0,0\n" + card2 + "\n" + card3
    ...
    def CreateContact(self, request, context):
        ...
        card2 = (
            str(fs) + "," + str(fd) + ",,," + str(vdc) + "," + str(penchk) + "," + str(birthtime)
        )
        ...
        if (
            ...
            or option1 == "NODES_TO_SURFACE"
            or option1 == "SURFACE_TO_SURFACE"
        ):
            newk = opcode + "\n" + card1 + "\n" + card2 + "\n" + card3
    ...
    def CreateBdyConvection(self, request, context):
        ...
        tmult = request.tmult
        loc = request.loc
        fields1 = (str(ssid),str(pserod))
        card1 = ",".join(fields1)
        fields2 = (str(hlcid),str(hmult),str(tlcid),str(tmult),str(loc))
        card2 = ",".join(fields2)
        ktuple = ("*BOUNDARY_CONVECTION_SET",card1,card2)
        newk = "\n".join(ktuple)
        self.kwdproc.newkeyword(newk)
        msg = "*BOUNDARY_CONVECTION_SET Created..."
        print(msg)
        return kwprocess_pb2.BdyConvectionReply(answer=0)
    ...
    def CreateMatPlasticKinematic(self, request, context):
        ...
        etan = request.etan
        beta = request.beta
        fields = (str(mid),str(ro),str(e),str(pr),str(sigy),str(etan),str(beta))
        card1 = ",".join(fields)
        opcode = "*MAT_PLASTIC_KINEMATIC"
        newk = opcode + "\n" + card1
        self.kwdproc.newkeyword(newk)
        msg = opcode + " Created..."
        print(msg)
        return kwprocess_pb2.MatPlasticKinematicReply(mid=mid)
    ...
    def CreateMatCrushableFoam(self, request, context):
        ...
        lcid = request.lcid
        tsl = request.tsl
        fields = (str(mid),str(ro),str(e),str(pr),str(lcid),str(tsl))
        card1 = ",".join(fields)
        opcode = "*MAT_CRUSHABLE_FOAM"
        newk = opcode + "\n" + card1
        self.kwdproc.newkeyword(newk)
        msg = opcode + " Created..."
        print(msg)
        return kwprocess_pb2.MatCrushableFoamReply(mid=mid)
    ...
    def CreateMatThermalOrthotropic(self, request, context):
        ...
        k3 = request.k3
        card1 = str(mid)
        fields = (str(hc),str(k1),str(k2),str(k3))
        card2 = ",".join(fields)
        opcode = "*MAT_THERMAL_ORTHOTROPIC"
        newk = opcode + "\n" + card1 + "\n" + card2
        self.kwdproc.newkeyword(newk)
        print(f"MAT_THERMAL_ISOTROPIC {mid} Created...")
        return kwprocess_pb2.MatThermalOrthotropicReply(mid=mid)
    ...
    def SetICFDVolumePartProperty(self, request, context):
        ...
        self.kwdproc.set_kwd_data(
            gdt.KWD_DETAILDATA_EXTERNALID, "ICFD_PART_VOL_KIND", pid, 4, secid
        )
    ...
    def CreateEMControlContact(self, request, context):
        ...
        dtype = request.dtype
        card1 = (
            str(emct) + "," + str(cconly) + "," + str(ctype) + "," + str(dtype) + ",0.3,0.3,0.3,,"
        )
    ...
    def CreateEMMat006(self, request, context):
        ...
        sigp = request.sigp
        sign = request.sign
        fields = (str(mid),str(mtype),str(sigp),"",str(sign))
        card1 = ",".join(fields)
        newk = "*EM_MAT_006\n" + card1
        self.kwdproc.newkeyword(newk)
        msg = "*EM_MAT_006 Created..."
        print(msg)
        return kwprocess_pb2.EMMat006Reply(mid=mid)

    def CreateEMIsopotential(self, request, context):
        isoid = self.kwdproc.get_data(gdt.KWD_EM_ISOPOTENTIAL_LASTID) + 1
        settype = request.settype
        setid = request.setid
        rdltype = request.rdltype
        card1 = str(isoid)+ ","+ str(settype)+ ","+ str(setid)+ ","+ str(rdltype)
    ...
    def CreateEMRandlesBatmac(self, request, context):
        rdlid = self.kwdproc.get_data(gdt.KWD_EM_RANDLES_LASTID) + 1
        rdltype = request.rdltype
        rdlarea = request.rdlarea
        psid = request.psid
        q = request.q
        cq = request.cq
        socinit = request.socinit
        soctou = request.soctou
        prm = request.chargedirparam
        temp = request.temp
        frther = request.frther
        r0toth = request.r0toth
        fields1 = (str(rdlid),str(rdltype),str(rdlarea),str(psid))
        card1 = ",".join(fields1)
        fields2 = (str(q),str(cq),str(socinit),str(-soctou))
        card2 = ",".join(fields2)
        fields3 = (str(prm[0]),str(prm[1]),str(prm[2]),str(prm[3]),str(prm[4]),str(prm[5]))
        card3 = ",".join(fields3)
        fields4 = (str(temp),str(frther),str(r0toth))
        card4 = ",".join(fields4)
        card5 = "0"
        ktuple = ("*EM_RANDLES_BATMAC",card1,card2,card3,card4,card5)
        newk = "\n".join(ktuple)
        self.kwdproc.newkeyword(newk)
        msg = "*EM_RANDLES_BATMAC Created..."
        print(msg)
        return kwprocess_pb2.EMRandlesBatmacReply(rdlid=rdlid)
    
    def CreateEMRandlesShort(self, request, context):
        func = request.function
        fields1 = (str(2),str(func))
        card1 = ",".join(fields1)
        ktuple = ("*EM_RANDLES_SHORT",card1)
        newk = "\n".join(ktuple)
        self.kwdproc.newkeyword(newk)
        msg = "*EM_RANDLES_SHORT Created..."
        print(msg)
        return kwprocess_pb2.EMRandlesShortReply(answer=0)
    
    def CreateEMRandlesExothermicReaction(self, request, context):
        areatype = request.areatype
        func = request.function
        fields1 = (str(areatype),str(func))
        card1 = ",".join(fields1)
        ktuple = ("*EM_RANDLES_EXOTHERMIC_REACTION",card1)
        newk = "\n".join(ktuple)
        self.kwdproc.newkeyword(newk)
        msg = "*EM_RANDLES_EXOTHERMIC_REACTION Created..."
        print(msg)
        return kwprocess_pb2.EMRandlesExothermicReactionReply(answer=0)
    ...
    def CreateEMSolverFemBemMonolithic(self, request, context):
        ...
        maxit = request.maxit
        card1 = (
            str(mtype) + "," + str(stype) + "," + str(abstol) + "," + str(reltol) + "," + str(maxit)
        )
