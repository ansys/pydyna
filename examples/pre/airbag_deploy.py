...
if __name__ == "__main__":
    ...
    airbag.control_energy(hgen=2,rwen=2,slnten=2)
    ...
    airbag.create_simple_airbag_model(modeltype="SIMPLE_AIRBAG_MODEL",sid=1,sidtyp=1,cv=1.736e3,cp=2.43e3,t=1.2e3,lcid=1,mu=0.7,area=0,pe=14.7,ro=3.821e-6)
    tail = [0,0,0]
    head = [0,1,0]
    airbag.create_rigidwall_planar(nsid=0,tail=tail,head=head,fric=0.5)
    airbag.create_contact(cid=1,title="airbag and the cylinder",
    option1="NODES_TO_SURFACE",option3=False,ssid=3,msid=2,sapr=1,sbpr=1,fs=0.5,fd=0.5,sfsb=0.06667)
    ...
    airbag.create_mat_fabric(mid=3,ro=1e-4,ea=2e6,eb=2e6,prba=0.35,prab=0.35,gab=1.53e6)
    thk = [0.5, 0.5, 0.5, 0.5]
    airbag.create_section_shell(
        secid=1, elform=0, thick=thk
    )
    thk = [0.015, 0.015, 0.015, 0.015]
    airbag.create_section_shell(
        secid=2, elform=9, thick=thk,nip=4
    )
